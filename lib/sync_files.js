...
class Sync {
  ...
  async connect() {
    ...
    this.remotePath = this.syncConf.transport.remotePath || `~/${this.dirName}`;
  ...
  async putFiles(fileList) {
    ...
    try {
      ...
      console.warn(colors.bold.red(`[${'putFiles'}]`), e.message);
    }
  }
    ...
    try {
      ...
      if (res.stdout) {
        res.stdout.toString().split(/\r?\n/).forEach(line => {
          console.log(colors.bold.green(`[${label}]`), line);
        });
      }
      if (res.stderr) {
        res.stderr.toString().split(/\r?\n/).forEach(line => {
          console.warn(colors.bold.yellow(`[${label}]`), line);
        });
      }
    } catch (e) {
      console.warn(colors.bold.red(`[${label}]`), e.message);
    }
  }
}
